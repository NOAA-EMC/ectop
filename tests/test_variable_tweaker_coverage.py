...
def mock_client():
    ...
    client = MagicMock()
    # Mock both async and sync versions
...
async def test_variable_tweaker_logic_node_not_found(mock_client):
    ...
    with patch.object(VariableTweaker, "app", new_callable=PropertyMock) as mock_app:
        ...
        mock_client.get_defs_sync.return_value.find_abs_node.return_value = None
...
async def test_variable_tweaker_submit_change(mock_client):
    ...
    with patch.object(VariableTweaker, "app", new_callable=PropertyMock) as mock_app:
        ...
        with patch.object(tweaker, "refresh_vars", new_callable=MagicMock):
            tweaker._submit_variable_logic("VAL1")
            mock_client.alter_sync.assert_called_with("/node", "add", "variable", "VAR1", "VAL1")
...
async def test_variable_tweaker_submit_add(mock_client):
    ...
    with patch.object(VariableTweaker, "app", new_callable=PropertyMock) as mock_app:
        ...
        tweaker.query_one = MagicMock()
        with patch.object(tweaker, "refresh_vars", new_callable=MagicMock):
            tweaker._submit_variable_logic("VAR2=VAL2")
            mock_client.alter_sync.assert_called_with("/node", "add", "variable", "VAR2", "VAL2")
